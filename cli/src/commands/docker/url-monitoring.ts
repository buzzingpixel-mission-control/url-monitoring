...
export default class UrlMonitoring extends Command {
    // Allow variable arguments
    public static strict = false;

    public static summary = `Execute command in the ${chalk.cyan('url-monitoring')} image. Empty argument starts a shell session`;
    ...
    public async run (): Promise<void> {
        ...
        this.log(chalk.yellow(
            "You're working inside the 'url-monitoring' application container of this project.",
        ));
        ...
        if (this.argv.length) {
            execSync(
                `
                    docker run -it --rm \
                        --entrypoint "" \
                        --name mission-control-url-monitoring \
                        -v "${rootDir}:/app" \
                        -w /app \
                        buzzingpixel/mission-control-url-monitoring bash -c "${this.argv.join(' ')}";
                `,
                { stdio: 'inherit' },
            );
        ...
        execSync(
            `
                docker run -it --rm \
                    ...
                    -v "${rootDir}:/app" \
                    -w /app \
                    buzzingpixel/mission-control-url-monitoring bash;
            `,
            { stdio: 'inherit' },
        );
    }
